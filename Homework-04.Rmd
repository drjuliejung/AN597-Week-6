...
ci = predict(m, newdata = data.frame(log(Brain_Size_Species_Mean) = v), interval = "confidence", level = 0.90)
pi = predict(m, newdata = data.frame(log(Brain_Size_Species_Mean) = v), interval = "prediction", level = 0.90)
df <- data.frame(cbind(v, ci, pi))
names(df) <- c("Brain_Size_Species_Mean", "CIfit", "CIlwr", "CIupr", "PIfit", "PIlwr", "PIupr")
head(df)
